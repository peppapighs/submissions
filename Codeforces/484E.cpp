...
struct node {
    ...
    int p, s, m, f;
    node() {}
    node(int l, int r, int p, int s, int m, int f)
        : l(l), r(r), p(p), s(s), m(m), f(f) {}
...
int update(int x, int p, int l = 1, int r = n) {
    ...
    int mid = (l + r) >> 1;
    if (x <= mid)
        t[p].l = update(x, t[p].l, l, mid);
    else
        t[p].r = update(x, t[p].r, mid + 1, r);
