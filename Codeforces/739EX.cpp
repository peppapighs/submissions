...
pair<int, int> solve(double C1, double C2) {
    for(int i = 1; i <= n; i++) {
        dp[i] = dp[i-1];
        if(dp[i].p < dp[i-1].p + pa[i] - C1) dp[i] = item(dp[i-1].p + pa[i] - C1, dp[i-1].a + 1, dp[i-1].b);
        if(dp[i].p < dp[i-1].p + pb[i] - C2) dp[i] = item(dp[i-1].p + pb[i] - C2, dp[i-1].a, dp[i-1].b + 1);
        if(dp[i].p < dp[i-1].p + pab[i] - C1 - C2) dp[i] = item(dp[i-1].p + pab[i] - C1 - C2, dp[i-1].a + 1, dp[i-1].b + 1);
    } 
...
int main() {
    ...
    for(int it1 = 1; it1 <= 100; it1++) {
        ...
        for(int it2 = 1; it2 <= 100; it2++) {
            mid2 = (l2 + r2) / 2.0;
            if(solve(mid1, mid2).second <= b) r2 = mid2;
            else l2 = mid2;
        }
        if(solve(mid1, r2).first <= a) r1 = mid1;
        else l1 = mid1;
    }
    solve(r1, r2);
    printf("%lf\n", dp[n].p + r2*b + r1*a);
