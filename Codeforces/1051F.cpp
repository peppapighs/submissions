...
void dfs(int u, int p) {
    ...
    for (pii v : g[u])
        if (v.x != p)
            dep[v.x] = dep[u] + 1, par[v.x][0] = u, d[v.x] = d[u] + v.y,
            dfs(v.x, u);
}
...
int main() {
    ...
    for (int i = 0, u, v, w; i < m; ++i) {
        ...
        if (a != b)
            P[a] = b, g[u].emplace_back(v, w), g[v].emplace_back(u, w);
        else
            E.emplace_back(u, v, w), coor.emplace_back(u), coor.emplace_back(v);
    }
    dfs(1, 0);
    ...
    for (int i = 0; i < coor.size(); ++i)
        for (int j = 1; j <= n; ++j)
            di[i][j] = getd(coor[i], j);
    for (int i = 0; i < coor.size(); ++i)
        for (int j = 0; j < coor.size(); ++j)
            ap[i][j] = min(ap[i][j], getd(coor[i], coor[j]));
    ...
    while (T--) {
        ...
        for (int i = 0; i < coor.size(); ++i)
            for (int j = 0; j < coor.size(); ++j)
                ans = min(ans, di[i][a] + di[j][b] + ap[i][j]);
        printf("%lld\n", ans);
    }
