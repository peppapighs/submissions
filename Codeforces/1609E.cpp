...
void find_merged() {
    for (int l = 0; l < 1 << 5; l++)
        for (int r = 0; r < 1 << 5; r++) {
            ...
            for (string &s_l : vec_l)
                for (string &s_r : vec_r) {
                    string now = s_l + s_r;
                    if (contains(now, "abc"))
                        merged[l][r] = -1;
                    else
                        for (int i = 0; i < 5; i++)
                            if (contains(now, sub[i]))
                                merged[l][r] |= (1 << i);
                }
        }
}
...
struct item {
    ...
    friend item operator+(const item &lhs, const item &rhs) {
        ...
        for (int i = 0; i < 1 << 5; i++)
            for (int j = 0; j < 1 << 5; j++) {
                int now = merged[i][j];
                if (now == -1 || lhs.dp[i] == -1 || rhs.dp[j] == -1)
                    continue;
                if (ret.dp[now] == -1)
                    ret.dp[now] = lhs.dp[i] + rhs.dp[j];
                else
                    ret.dp[now] = min(ret.dp[now], lhs.dp[i] + rhs.dp[j]);
            }
        return ret;
    }
};
