...
int get(int x) { return lower_bound(leaf.begin(), leaf.end(), x) - leaf.begin(); }
...
int proc(int u, int p) {
    int now = u;
    if(g[u].size() == 1) leaf.emplace_back(u);
...
void dfs(int u, int p) {
    for(iii now : Q[u]) {
        ...
        tie(l, r, idx) = now;
        ans[idx] = LLONG_MAX;
        travel(get(l), get(r) - (leaf[get(r)] != r), [&](var) { ans[idx] = min(ans[idx], t[p]); });
    }
    for(pii v : g[u]) if(v.x != p) {
        ...
        travel(get(v.x), get(out[v.x]) - (leaf[get(out[v.x])] != out[v.x]), [&](var) {
            lz[p] -= 2*v.y;
            push(p, l, r);
        });
        dfs(v.x, u);
