...
void gen_bct() {
    ...
    for(vector<int> &cc : bcc) {
        g.emplace_back(0);
        for(int u : cc) {
            if(art[u]) g[g.size() - 1].emplace_back(id[u]), g[id[u]].emplace_back(g.size() - 1);
            else id[u] = g.size() - 1, Q[g.size() - 1].add(w[u]);
        }
    }
    sz = g.size() - 1;
    dfs(1, 0);
    for(int i = 1; i <= n; i++) if(art[i]) Q[par[id[i]]].add(w[i]);
    for(int i = 1, idx = 0; i <= sz; i++) if(spi[par[i]] != i)
        for(int j = i; j != -1; j = spi[j])
            rot[j] = i, pos[j] = ++idx;
    for(int i = 1; i <= sz; i++) tree.update(pos[i], Q[i].top());
}
        ...
        if(T == 'A') {
            if(a == b) printf("%d\n", w[a]);
                ...
                if(a == b) {
                    ...
                    if(!idart[a] && par[a]) now = min(now, Q[par[a]].top());
                    printf("%d\n", now);
                } else {
