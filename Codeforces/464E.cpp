...
int build(bool inf, int l = 0, int r = N - 1) {
    if (l == r)
        return new_leaf(inf ? mpow[l] : Hash(0));
    int mid = (l + r) >> 1;
...
bool get_pos(int x, int p, int l = 0, int r = N - 1) {
    ...
    int mid = (l + r) >> 1;
    if (x <= mid)
        return get_pos(x, t[p].l, l, mid);
    else
        return get_pos(x, t[p].r, mid + 1, r);
}

int increment(int x, int p, int l = 0, int r = N - 1) {
    if (l == r)
        return new_leaf(t[p].h.empty() ? mpow[l] : Hash(0));
    int mid = (l + r) >> 1;
    if (x <= mid)
        return new_par(increment(x, t[p].l, l, mid), t[p].r);
    else
        return new_par(t[p].l, increment(x, t[p].r, mid + 1, r));
}
...
bool compare(int p1, int p2, int l = 0, int r = N - 1) {
    if (l == r)
        return (t[p1].h.empty() && !t[p2].h.empty());
    int mid = (l + r) >> 1;
    if (t[t[p1].r].h == t[t[p2].r].h)
        return compare(t[p1].l, t[p2].l, l, mid);
    else
        return compare(t[p1].r, t[p2].r, mid + 1, r);
}
