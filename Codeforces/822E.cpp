...
int main() {
    ...
    sz = n + m + 1;
    for(int i = 0; i < sz; i++) suf[i] = i, pos[i] = A[i];
    for(int k = 1; k <= sz; k <<= 1) {
        auto cmp = [&](int a, int b) {
            if(pos[a] != pos[b]) return pos[a] < pos[b];
            if(a + k < sz && b + k < sz) return pos[a + k] < pos[b + k];
            return a > b;
        };
        sort(suf, suf + sz, cmp);
        int tmp[N]; tmp[0] = 0;
        for(int i = 1; i < sz; i++) tmp[i] = tmp[i-1] + cmp(suf[i-1], suf[i]);
        for(int i = 0; i < sz; i++) pos[suf[i]] = tmp[i];
    }
