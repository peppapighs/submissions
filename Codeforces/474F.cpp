...
int query(int x, int y, int p = 1, int l = 1, int r = n) {
    ...
    if (x <= l && r <= y)
        return tree[p];
    int mid = (l + r) >> 1;
    return __gcd(query(x, y, p << 1, l, mid),
...
int main() {
    ...
    while (m--) {
        ...
        scanf("%d%d", &a, &b);
        int gcd = query(a, b);
        if (comp.find(gcd) == comp.end())
            printf("%d\n", b - a + 1);
        else {
            int cnt =
                upper_bound(loc[comp[gcd]].begin(), loc[comp[gcd]].end(), b) -
                lower_bound(loc[comp[gcd]].begin(), loc[comp[gcd]].end(), a);
            printf("%d\n", b - a + 1 - cnt);
        }
        // printf("%d\n",gcd);
    }
