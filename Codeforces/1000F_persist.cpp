...
node *newleaf(pii a) {
	return new node(a, nullptr, nullptr);
}

node *newpar(node *l, node *r) {
	return new node(min(l->d, r->d), l, r);
}

int n, m, arr[N], pos[N];

node *build(int l = 1, int r = n) {
	if(l == r) return newleaf(pii(INT_MAX, 0));
	int mid = (l + r) >> 1;
...
int main() {
	...
	for(int i = 1;i <= n;i++) {
		...
		if(pos[arr[i]]) ret = update(pos[arr[i]], pii(INT_MAX, 0), persist[i - 1]);
