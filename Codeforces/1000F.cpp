...
void update(int x,int a,int b,int p = 1,int l = 1,int r = n) {
	if(l == r) return void(tree[p] = {a,b});
	int mid = (l + r) >> 1;
	if(x <= mid) update(x,a,b,p<<1,l,mid);
	else update(x,a,b,p<<1|1,mid+1,r);
	tree[p] = min(tree[p<<1],tree[p<<1|1]);
}

ii query(int x,int y,int p = 1,int l = 1,int r = n) {
	if(x > r || l > y) return {INT_MAX,-1};
...
int main() {
	...
	for(int i = 1;i <= n;i++) {
		if(pos[arr[i]]) update(pos[arr[i]],INT_MAX,-1);
		update(i,pos[arr[i]],arr[i]);
		pos[arr[i]] = i;
		for(ii q : que[i]) {
			ii mn = query(q.first,i);
			if(mn.first < q.first) ans[q.second] = mn.second;
		}
	}
