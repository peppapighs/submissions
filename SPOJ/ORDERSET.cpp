...
void updt(pitem t) { if(t) t->size = sz(t->l) + 1 + sz(t->r); }
...
bool find(pitem t, int key) {
	...
	else if(t->val < key) return find(t->r, key);
...
int kth(pitem t, int x, int add = 0) {
	...
	if(cur_pos == x) return t->val;
	else if(cur_pos < x) return kth(t->r, x, cur_pos);
	else return kth(t->l, x, add);
}

int count(pitem t, int x) {
	if(!t) return 0;
	if(t->val < x) return count(t->r, x) + sz(t->l) + 1;
