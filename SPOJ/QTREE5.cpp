...
void hld() {
    proc(1, 0);
    for (int i = 1, idx = 0, ch = 0; i <= n; i++)
        if (spi[par[i]] != i) {
            ++ch;
            for (int j = i; j != -1; j = spi[j])
                pos[j] = idx++, rot[j] = i, id[j] = ch, ++sz[ch],
                t[pos[j] + n] = pii(1e8, 1e8);
        }
    for (int i = n - 1; i; i--)
...
void color(int a) {
    ...
    while (a) {
        ...
        update(pos[a],
               pii(*S[a].begin() + pos[a] - pos[rot[a]],
                   *S[a].begin() + sz[id[a]] - 1 - pos[a] + pos[rot[a]]));
...
int query(int a) {
    int ret = 1e8, dist = 0;
    while (a) {
        ret = min(ret, tquery(pos[rot[a]], pos[a]).y + dist -
                           (sz[id[a]] - 1 - pos[a] + pos[rot[a]]));
        ret = min(ret, tquery(pos[a], pos[rot[a]] + sz[id[a]] - 1).x + dist -
                           (pos[a] - pos[rot[a]]));
