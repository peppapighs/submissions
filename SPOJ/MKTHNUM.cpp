...
int query(int k, node *p1, node *p2, int l = 1, int r = coord.size(),
          int add = 0) {
    if (l == r)
        return l;
    int mid = (l + r) >> 1;
    ...
    if (cur_pos >= k)
        return query(k, p1->l, p2->l, l, mid, add);
    else
        return query(k, p1->r, p2->r, mid + 1, r, cur_pos);
}
