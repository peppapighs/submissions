...
void dfs(int u, int p) {
    ...
    for(int i = 1; i <= 16; i++) par[u][i] = par[par[u][i-1]][i-1];
    rot[u] = update(get(A[u]), rot[p]);
    for(int v : g[u]) if(v != p) dfs(v, u);
}

int query(int k, node* pu, node* pv, node* pl, node* pp, int add = 0, int l = 1, int r = coord.size()) {
    if(l == r) return l;
    int sum = add + pu->l->d + pv->l->d - pl->l->d - pp->l->d;
    int mid = (l + r) >> 1;
    if(sum >= k) return query(k, pu->l, pv->l, pl->l, pp->l, add, l, mid);
    else query(k, pu->r, pv->r, pl->r, pp->r, sum, mid+1, r);
}
