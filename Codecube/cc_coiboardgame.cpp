...
int main() {
	...
	while(!q.empty()) {
		...
		q.pop();
		for(int i=0;i<4;i++) {
			if(f.x+dx[i]>=0&&f.x+dx[i]<m&&f.y+dy[i]>=0&&f.y+dy[i]<n) {
				node u(f.x+dx[i],f.y+dy[i],f.bit);
				if(d[u.x][u.y]!='#') {
					if(d[u.x][u.y]>='a'&&d[u.x][u.y]<='z') {
						if(d[u.x][u.y]=='j') u.bit|=1;
						else if(d[u.x][u.y]=='b') u.bit|=2;
						else if(d[u.x][u.y]=='p') u.bit|=4;
						...
						bool chk=false;
						if(d[u.x][u.y]=='J'&&u.bit&1) chk=true;
						else if(d[u.x][u.y]=='B'&&(u.bit>>1)&1) chk=true;
						else if(d[u.x][u.y]=='P'&&(u.bit>>2)&1) chk=true;
