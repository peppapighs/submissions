...
void push1(var) {
    if(!lz1[p]) return;
    if(lz1[p] == 1) t1[p] = r - l + 1;
    else t1[p] = 0;
    if(l != r) lz1[p<<1] = lz1[p<<1|1] = lz1[p];
    lz1[p] = 0;
}
...
void push2(var) {
    if(!lz2[p]) return;
    if(lz2[p] == 1) t2[p] = r - l + 1;
    else t2[p] = 0;
    if(l != r) lz2[p<<1] = lz2[p<<1|1] = lz2[p];
    lz2[p] = 0;
}
...
int main() {
    ...
    findcyc(1, 1);
    int csz = 0;
    for(int u : cyc) dfs(u, u, u, ++csz);
    hld(); 
    for(int i = 1, a, b, c; i <= m; i++) {
        scanf("%d %d %d", &a, &b, &c);
        c = c ? -1 : 1;
        if(cid[a] == cid[b]) walk(a, b, c);
        else {
            if(cid[a] > cid[b]) swap(a, b);
            int pa = hd[a], pb = hd[b];
            walk(a, pa, c), walk(b, pb, c);
            if(2 * (cid[b] - cid[a]) <=  csz) update2(cid[a], cid[b]-1, c);
            if(2 * (cid[b] - cid[a]) >=  csz) update2(1, cid[a]-1, c), update2(cid[b], csz, c);
        }
