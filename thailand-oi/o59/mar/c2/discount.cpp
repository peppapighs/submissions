...
void dfs(int u, int p) {
    ...
    for(pii v : g[u]) if(v.x != p) mx[v.x][0] = v.y, dfs(v.x, u);
}

int lca(int a, int b) {
    int ret = 0;
    if(dep[a] < dep[b]) swap(a, b);
    for(int i = 15; ~i; i--) if(dep[par[a][i]] >= dep[b]) {
        ret = max(ret, mx[a][i]);
        a = par[a][i];
    }
    if(a == b) return ret;
    for(int i = 15; ~i; i--) if(par[a][i] != par[b][i]) {
...
int main() {
    ...
    for(iii e : mst) {
        ...
        if(pa != pb) dsu[pa] = pb, g[a].emplace_back(b, c), g[b].emplace_back(a, c);
