...
void update(long t[], int x, int k) {
    for (int i = x; i < N; i += i & -i)
        t[i] += k;
}
long query(long t[], int x, long ret = 0) {
    for (int i = x; i; i -= i & -i)
        ret += t[i];
    return ret;
}
...
void dfs(int u, int p, int ep, int mx, bool keep) {
    ...
    if (fac[u])
        update(t2, get(mxval[u]), 1);

    if (weight[ep] == mx)
        cnt[ep] += 1ll * query(t1, get(mx)) * query(t2, get(mx));
