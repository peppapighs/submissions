...
struct item {
    ...
    item *l, *r;
    item(int val) : val(val), l(NULL), r(NULL), inc(0), inv(0), prio(rand()), sz(1), lz(0), st(0) {
...
template<typename T>
void proc(int a, int b, const T &f) {
    pitem l, r;
...
int main() {
    scanf("%d %d", &n, &q);
    scanf(" %s", A+1);
    for(int i = 1; i <= n; i++) merge(t, t, new item(A[i] - '0'));
    ...
    while(q--) {
        ...
        } else if(T == 2) proc(a, b, [&](pitem t) { if(t->st != 2) t->st ^= 1; });
        else proc(a, b, [&](pitem t) { printf("%lld\n", t->inv); });
    }
