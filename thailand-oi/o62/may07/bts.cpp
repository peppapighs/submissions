...
void update(int x, int y, int k, int p = 1, int l = 1, int r = n-1) {
    ...
    if(x <= l && r <= y) {
        ...
        else if(l != r) t[p].x = t[p<<1].x + t[p<<1|1].x;
        else t[p].x = 0;
        return;
    }
    int mid = (l + r) >> 1;
...
bool chk(int mid) {
    for(int i = mid; i < n; i++) {
        int cnt = 0;
        for(int j = 1; j <= m; j++) if(i - mid + 1 <= u[j] && v[j] <= i + 1)
            ++cnt;
...
int main() {
    ...
    for(int i = r; i < n; i++) {
        vector<int> V;
        int cnt = 0;
        for(int j = 1; j <= m; j++) if(i - r + 1 <= u[j] && v[j] <= i + 1)
            update(u[j], v[j]-1, 1), V.emplace_back(j), ++cnt;
        if(cnt >= k) mn = min(mn, t[1].x);
        for(int j : V) update(u[j], v[j]-1, -1);
    }
    printf("%d\n", a * mn + b * (n - 1 - mn));
