...
int main() {
    ...
    for(int len = 1; len <= n; len++) for(int i = len, j = 1; i <= n; i++, j++) {
        int mx = 0;
        for(int k = 1; k <= m; k++) {
            mx += col[i][k] - col[j-1][k];
            drow[i][j][k] = max({drow[i][j][k-1], drow[i-1][j][k], drow[i][j+1][k]});
    ...
    for(int len = 1; len <= m; len++) for(int i = len, j = 1; i <= m; i++, j++) {
        int mx = 0;
        for(int k = 1; k <= n; k++) {
            mx += row[k][i] - row[k][j-1];
            dcol[i][j][k] = max({dcol[i][j][k-1], dcol[i-1][j][k], dcol[i][j+1][k]});
