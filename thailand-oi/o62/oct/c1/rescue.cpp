...
int main() {
    ...
    if(cnt > 1) for(int i = 0; i < 4; i++) {
        pii p1 = vec[i], p2 = vec[(i + 1) % 4];
        int a = ret[i], b = ret[(i + 1) % 4];
        if(a != -1 && b != -1) solve(p1, p2, a, b);
    } else {
        if(ret[0] != -1) {
            int l = 1, r = C;
            while(l < r) {
                int mid = (l + r + 1) >> 1;
                if(drop_robot(1, mid) != -1) l = mid;
                else r = mid - 1;
            }
            C = l;
            l = 1, r = R;
