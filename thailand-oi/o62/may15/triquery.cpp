...
long rect1(int x1, int y1, int x2, int y2) {
    return query(t1, x2, y2) - query(t1, x1 - 1, y2) - query(t1, x2, y1 - 1) + query(t1, x1 - 1, y1 - 1);
}

long rect2(int x1, int y1, int x2, int y2) {
    return query(t2, x2, y2) - query(t2, x1 - 1, y2) - query(t2, x2, y1 - 1) + query(t2, x1 - 1, y1 - 1);
}

int main() {
    scanf("%d %d", &n, &m);
    for(int sum = 2; sum <= 2 * n; sum++) for(int i = 1; i < sum && i <= n; i++) {
        if(sum - i < 1 || sum - i > n) continue;
        r[i][sum - i] = sum - 1;
        c[i][sum - i] = n - i + 1;
    }
    for(int i = 1, t, x, y, k; i <= m; i++) {
        ...
        if(t == 1) {
            ...
            if(k > 1) {
                ans += rect1(r[x1][y1 + 1], c[x1][y1 + 1], 2 * n - 1, c[x2 + 1][y2]);
