...
template<typename T>
void travel(int x, int y, const T &f, int p = 1, int l = 1, int r = coord.size()) {
    push(p, l, r);
    if(x > r || l > y) return;
    if(x <= l && r <= y) return f(p, l, r);
    int mid = (l + r) >> 1;
    travel(x, y, f, p<<1, l, mid), travel(x, y, f, p<<1|1, mid+1, r);
    for(int i = 0; i <= 3; i++) t[i][p] = max(t[i][p<<1], t[i][p<<1|1]);
}
