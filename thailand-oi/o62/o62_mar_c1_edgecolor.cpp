...
struct item {
    ...
    int d, lz, sz, prio;
    item *l, *r, *p;
    item(int d) : tag(false), d(d), lz(0), sz(1), prio(rand()), l(NULL), r(NULL), p(NULL) { }
};
...
int getpos(pitem t) {
    vector<bool> vec;
    while(t->p) {
        if(t->p->l == t) vec.emplace_back(0);
        else vec.emplace_back(1);
        t = t->p;
    }
    int add = 0;
    for(int i = vec.size()-1; ~i; i--) {
        if(t->lz) push(t), vec[i] = !vec[i];
        if(!vec[i]) t = t->l;
        else add += sz(t->l) + 1, t = t->r;
    }
    push(t);
    return add + sz(t->l) + 1;
}
...
int main() {
    scanf("%d %d", &n, &m);
    for(int i = 1; i <= n; i++) t[i] = new item(i);
    for(int i = 1, T, a, b; i <= m; i++) {
        ...
        if(T == 1) {
            if(root(t[a]) != root(t[b])) {
                ...
                if(getpos(t[b]) == sz(root(t[b]))) root(t[b])->lz ^= 1;
                merge(tmp, root(t[a]), root(t[b]));
                tmp->p = NULL;
            } else {
                root(t[a])->tag = true;
                if(sz(root(t[a])) & 1) ++cnt;
            }
        } else {
