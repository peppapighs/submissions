...
int query(int x, int y, node* p1, node* p2, int l = 1, int r = coord.size()) {
    if(x > r || l > y) return 0;
    if(x <= l && r <= y) return p1->d - p2->d;
    int mid = (l + r) >> 1;
    return query(x, y, p1->l, p2->l, l, mid) + query(x, y, p1->r, p2->r, mid+1, r);
}
...
int main() {
    scanf("%d", &n);
    for(int i = 1; i <= n; i++) scanf("%d", X+i), coord.emplace_back(X[i]);
    for(int i = 1; i <= n; i++) scanf("%d", D+i);
    sort(coord.begin(), coord.end());
    coord.resize(unique(coord.begin(), coord.end()) - coord.begin());
    l[0] = r[n+1] = build();
    for(int i = 1; i <= n; i++) l[i] = update(get(X[i]), l[i-1]);
    for(int i = n; i; i--) r[i] = update(get(X[i]), r[i+1]);
    for(int i = 1; i <= n; i++) {
