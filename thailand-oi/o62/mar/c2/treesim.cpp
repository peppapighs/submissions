...
long dfs(int u, int p, int i) {
    ...
    if (u != p)
        hsh[i][u].emplace_back(all - ret, p);
    return ret;
}
...
int main() {
    ...
    mpow[0] = 1;
    for (int i = 1; i <= n; i++)
        mpow[i] = mpow[i - 1] * iden, all += mpow[i];
    ...
    for (pii e : E[0]) {
        int a = e.x, b = e.y;
        long ha = g[0][a].size() == 1 ? mpow[a] : 0,
             hb = g[0][b].size() == 1 ? mpow[b] : 0;
        for (pii h : hsh[0][a])
            if (h.y != b)
                ha += h.x;
        for (pii h : hsh[0][b])
    ...
    for (pii e : E[1]) {
        int a = e.x, b = e.y;
        long ha = g[1][a].size() == 1 ? mpow[a] : 0,
             hb = g[1][b].size() == 1 ? mpow[b] : 0;
        for (pii h : hsh[1][a])
            if (h.y != b)
                ha += h.x;
        for (pii h : hsh[1][b])
