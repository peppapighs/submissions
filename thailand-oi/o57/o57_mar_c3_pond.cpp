...
void dfs(int r, int c, int bit) {
    if(bit == (1 << idx) - 1) return void(ans = min(ans, dp[r][c][bit]));
    for(int i = 0; i < 4; i++) {
        int nr = r + dx[i], nc = c + dy[i], nbit = bit;
        long sw = 0;
        if(nr < 1 || nr > h || nc < 1 || nc > w || col[nr][nc] == -1) continue;
        if(col[nr][nc] > 0) nbit |= (1 << (col[nr][nc] - 1));
...
int main() {
    ...
    for(int i = 1; i <= h; i++) {
        if(!A[i][1] && col[i][1] == -2) flood(i, 1, -1);
        if(!A[i][w] && col[i][w] == -2) flood(i, w, -1);
    }
    for(int i = 1; i <= w; i++) {
        if(!A[1][i] && col[1][i] == -2) flood(1, i, -1);
        if(!A[h][i] && col[h][i] == -2) flood(h, i, -1);
    }
    for(int i = 1; i <= h; i++) for(int j = 1; j <= w; j++) {
        if(A[i][j] && col[i][j] == -2) flood(i, j, 0);
