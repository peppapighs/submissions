...
void del(int i, int v) {
	if(!book.count(v)) return;
	for(pil& p : book[v]) {
		if(find(p.x) != i) continue;
		p.x = 0;
		llist *node = p.y;
		if(node->l != NULL) node->l->r = node->r;
		if(node->r != NULL) node->r->l = node->l;
		if(node == H[i]) H[i] = node->r;
		if(node == T[i]) T[i] = node->l;
	}
}
...
int main() {
	...
	for(int i = 1; i <= k; i++) for(llist *cur = H[i]; cur != NULL; cur = cur->r) printf("%d\n", cur->val);
