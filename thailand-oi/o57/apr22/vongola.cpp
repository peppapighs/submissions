...
void dfs(int u, int p, bool keep) {
    ...
    for (pii x : Q[u]) {
        int d = x.x, k = x.y;
        int a = lower_bound(C[dep[u] + d].begin(), C[dep[u] + d].end(),
                            mn[dep[u] + d]) -
                C[dep[u] + d].begin();
        int b = lower_bound(C[dep[u] + d].begin(), C[dep[u] + d].end(),
                            mx[dep[u] + d]) -
                C[dep[u] + d].begin();
        dp[dep[u] + d][a] += k;
        if (b + 1 < C[dep[u] + d].size())
            dp[dep[u] + d][b + 1] -= k;
        A[u] -= 1ll * k * (b - a + 1);
    }
    if (!keep)
        for (int i = in[u]; i <= out[u]; i++)
            mn[dep[pos[i]]] = 1e9, mx[dep[pos[i]]] = 0;
}
...
int main() {
    ...
    for (int i = 1; i <= n; i++)
        dp.emplace_back(vector<long>(C[i].size(), 0));
    dfs(0, 0, 1);
    for (int i = 1; i <= n; i++) {
        if (!C[i].empty())
            A[pos[C[i][0]]] += dp[i][0];
