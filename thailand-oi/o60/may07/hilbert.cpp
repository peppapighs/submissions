...
long dist(pii p, int n) {
    ...
    if (n & 1) {
        if (a < ca && b < cb)
            return dist(pii(a, b), n - 1);
        if (a < ca && b >= cb)
            return ndist + 1 + ndist -
                   dist(rot(pii(a, b - cb + 1), n - 1, 4), n - 1);
        if (a >= ca && b >= cb)
            return 2ll * ndist + 2 + ndist -
                   dist(rot(pii(a - ca + 1, b - cb + 1), n - 1, 4), n - 1);
        if (a >= ca && b < cb)
            return 3ll * ndist + 3 +
                   dist(rot(pii(a - ca + 1, b), n - 1, 3), n - 1);
    } else {
        if (a < ca && b < cb)
            return dist(pii(a, b), n - 1);
        if (a >= ca && b < cb)
            return ndist + 1 + ndist -
                   dist(rot(pii(a - ca + 1, b), n - 1, 2), n - 1);
        if (a >= ca && b >= cb)
            return 2ll * ndist + 2 + ndist -
                   dist(rot(pii(a - ca + 1, b - cb + 1), n - 1, 2), n - 1);
        if (a < ca && b >= cb)
            return 3ll * ndist + 3 +
                   dist(rot(pii(a, b - cb + 1), n - 1, 3), n - 1);
    }
}
