...
int get(int i, int x1, int x2, int y1, int y2) {
    if (x1 > x2 || y1 > y2)
        return 0;
    return dp[i][x2][y2] - dp[i][x1 - 1][y2] - dp[i][x2][y1 - 1] +
           dp[i][x1 - 1][y1 - 1];
}
...
int sum(int x1, int x2, int y1, int y2) {
    if (x1 > x2 || y1 > y2)
        return 0;
    return qs[x2][y2] - (x1 > 0 ? qs[x1 - 1][y2] : 0) -
           (y1 > 0 ? qs[x2][y1 - 1] : 0) +
           (x1 > 0 && y1 > 0 ? qs[x1 - 1][y1 - 1] : 0);
}
...
int main() {
    ...
    for (int k = 0; k < 3; k++)
        for (int i = 1; i <= n + 1; i++)
            for (int j = 1; j <= m + 1; j++)
                dp[k][i][j] +=
                    dp[k][i - 1][j] + dp[k][i][j - 1] - dp[k][i - 1][j - 1];
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= m; j++)
        ...
        if (r2 - r1 > 1 && c2 - c1 > 1) {
            for (int i = r1; i <= r2; i++) {
                if (A[i][c1] && !chk[cc[i][c1]]) {
                    pii a = vec[cc[i][c1] - 1];
                    if (inside(a, r1 + 1, r2 - 1, c1 + 1, c2 - 1))
                        --c;
                    chk[cc[i][c1]] = 1;
                }
            ...
            for (int i = r1; i <= r2; i++)
                chk[cc[i][c1]] = chk[cc[i][c2]] = 0;
            for (int i = c1; i <= c2; i++)
                chk[cc[r1][i]] = chk[cc[r2][i]] = 0;
        }
