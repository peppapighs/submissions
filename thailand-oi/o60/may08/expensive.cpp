...
void dfs(int u, int p, int id, int ptr, long len) {
    if (coord[get(u) - 1] == u)
        return void(ap[id][get(u)] = min(ap[id][get(u)], len));
    if (!get<0>(d[u][0]))
        d[u][0] = iii(id, ptr, len);
    else
        d[u][1] = iii(id, ptr, len);
    for (pii v : g[u])
        if (v.x != p)
            dfs(v.x, u, id, ptr, len + v.y);
}
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++) {
                ...
                if (ida == -1 || idb == -1 || !ida || !idb)
                    continue;
                if (pa == pb)
                    ret = min(ret, abs(la - lb));
                else
                    ret = min(ret, la + ap[ida][idb] + lb);
            }
        printf("%lld\n", ret);
    }
