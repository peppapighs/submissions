...
struct item {
    int pos, d, sum, prio;
    item *l, *r;
    item(int pos, int d) : pos(pos), d(d), sum(d), prio(rand()), l(NULL), r(NULL) { }
};

typedef item* pitem;

int sum(pitem t) { return t ? t->sum : 0; }
void updt(pitem t) { if(t) t->sum = sum(t->l) + t->d + sum(t->r); }

void split(pitem t, pitem &l, pitem &r, int key) {
    if(!t) return void(l = r = NULL);
    if(t->pos <= key) split(t->r, t->r, r, key), l = t;
