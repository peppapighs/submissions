...
int solve(int bit) {
        ...
        if(~ret) return ret;
        ret = INT_MAX;
        for(int i = 0; i < odd.size(); i++) if((bit>>i) & 1) {
...
int main() {
        ...
        for(int i = 1; i <= n; i++) if(deg[i] & 1) odd.emplace_back(i);
        for(int i = 0; i < odd.size(); i++) {
                ...
                while(!Q.empty()) {
                        ...
                        int u = now.y;
                        for(int v : g[u]) if(dist[v] > dist[u] + 1) Q.emplace(dist[v] = dist[u] + 1, v); 
                }
                for(int j = 0; j < odd.size(); j++) ap[i][j] = dist[odd[j]];
        }
