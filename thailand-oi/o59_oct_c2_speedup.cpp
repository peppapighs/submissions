...
int main() {
	...
	for(int i = 1, a; i <= l; i++) scanf("%d", &a), potion[a] = 1;
	for(int i = 0; i < N; i++) for(int j = 0; j < 10; j++) dp[i][j] = LLONG_MAX;
	...
	while(!Q.empty()) {
		...
		for(pii vw : g[u]) {
			...
			if(pot < q && potion[u] && ls != u && dp[u][pot] + (w >> (pot + 1)) < dp[v][pot + 1]) {
	...
	for(int i = 0; i <= q; i++) ans = min(ans, dp[n][i]);
	printf("%lld", ans);
