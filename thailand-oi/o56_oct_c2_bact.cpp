...
int main() {
       ...
       scanf("%d %d %d", &l, &w, &m);
       for(int i = 1; i <= l; i++) for(int j = 1; j <= w; j++) {
              scanf("%d", &A[i][j]);
              ++cnt[A[i][j]];
       }
       for(int len = 1; len <= w; len++) for(int i = 1; i <= l; i++) { 
              memset(win, 0, sizeof win), alive = 0;
              for(int j = 1; j <= len; j++) add(i, j);
              rig[i][1][len] = alive;
       ...
       for(int len = 1; len <= l; len++) for(int j = 1; j <= w; j++) { 
              memset(win, 0, sizeof win), alive = 0;
              for(int i = 1; i <= len; i++) add(i, j);
              dwn[1][j][len] = alive;
