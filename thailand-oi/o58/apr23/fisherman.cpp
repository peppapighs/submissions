...
long find_max(long x) {
    ...
    while(l < r) {
        int mid = (l + r) >> 1;
        if(f(hi[mid], x) > f(hi[mid + 1], x)) r = mid;
        else l = mid + 1;
    }
    return f(hi[r], x);
}
...
long find_min(long x) {
    ...
    while(l < r) {
        int mid = (l + r) >> 1;
        if(f(lo[mid], x) < f(lo[mid + 1], x)) r = mid;
        else l = mid + 1;
    }
    return f(lo[r], x);
}
...
int main() {
    ...
    for(int i = 1, a, b; i <= m; i++) {
        scanf("%d %d", &a, &b);
        if(a > 0) {
            if(find_max(a) <= -b) printf("ALL\n");
            else if(find_min(a) > -b) printf("NONE\n");
            else printf("SOME\n");
        } else {
            if(find_min(a) >= -b) printf("ALL\n");
            else if(find_max(a) < -b) printf("NONE\n");
            else printf("SOME\n");
        }
    }
