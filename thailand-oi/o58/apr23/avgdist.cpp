...
void centroid(int u, int p) {
    ...
    dfs(u, u, dep[u]);
    for(pii v : g[u]) if(!chk[v.x]) centroid(v.x, u);
}
...
void getans(int a, bool del) {
    ...
    for(int i = par[a], pv = a; i; pv = i, i = par[i]) {
        ...
        ans[A[a]] += (d[a][dep[i]] * (cnt1[A[a]][i] - cnt2[A[a]][pv])) * (del ? -1ll : 1ll);
    }
}

void update(int a, bool del) {
    if(del) --col[A[a]], --cnt1[A[a]][a];
    else ++col[A[a]], ++cnt1[A[a]][a];
    for(int i = par[a], pv = a; i; pv = i, i = par[i]) {
        sum1[A[a]][i] += d[a][dep[i]] * (del ? -1ll : 1ll);
        sum2[A[a]][pv] += d[a][dep[i]] * (del ? -1ll : 1ll);
        if(del) --cnt1[A[a]][i], --cnt2[A[a]][pv];
        else ++cnt1[A[a]][i], ++cnt2[A[a]][pv]; 
    }
}
...
int main() {
    ...
    centroid(1, 0);
    for(int i = 1; i <= n; i++) getans(i, 0), update(i, 0);
    scanf("%d", &q);
