...
int main() {
    fill_n(dp[0], N * N, 1e12), fill_n(tr[0], N * (N << 1), 1e12),
        fill_n(tc[0], N * (N << 1), 1e12);
    ...
    for (int i = 1; i <= R; i++)
        for (int j = 1; j <= C; j++) {
            ...
            if (i == 1 && j == 1)
                ...
                dp[i][j] = min(row[i][j - 1] + query(tr[i], 0, idR - 1),
                               col[i - 1][j] + query(tc[j], 0, idC - 1));
            update(tr[i], idR, dp[i][j] - row[i][j]),
                update(tc[j], idC, dp[i][j] - col[i][j]);
        }
    if (dp[R][C] == 1e12)
        printf("-1\n");
    else
