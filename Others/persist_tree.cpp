...
item *newleaf(int v) { return new item(v, nullptr, nullptr); }

item *newpar(item *l, item *r) { return new item(l->v + r->v, l, r); }
...
item *update(int x, int v, item *p, int l = 1, int r = n) {
    if (l == r)
        return newleaf(p->v + v);
    int m = l + r >> 1;
    if (x <= m)
        return newpar(update(x, v, p->l, l, m), p->r);
    else
        return newpar(p->l, update(x, v, p->r, m + 1, r));
}
...
int main() {
    scanf("%d %d %d", &n, &m, &q);
    for (int i = 1; i <= n; ++i)
        scanf("%d", pref + i), pref[i] += pref[i - 1];
    ver[0] = build();
