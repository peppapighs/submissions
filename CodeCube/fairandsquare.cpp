...
int main() {
    ...
    for (int l = 1; l <= n; l++) {
        ...
        for (int i = l; i <= n; i++) {
            ...
            long cntl = query(t1, get(med) - 1),
                 cntr = query(t1, coord.size()) - query(t1, get(med));
            long suml = query(t2, get(med) - 1),
                 sumr = query(t2, coord.size()) - query(t2, get(med));
    ...
    for (int x = 1; x <= k; x++)
        for (int i = 1; i <= n; i++) {
            dp[i][x] = 1e18;
            for (int j = 0; j < i; j++)
                dp[i][x] = min(dp[i][x], dp[j][x - 1] + proc[j + 1][i]);
        }
    long ans = 1e18;
    for (int i = 1; i <= k; i++)
        ans = min(ans, dp[n][i]);
