...
struct item {
    ...
    friend item operator+(const item &lhs, const item &rhs) {
        ...
        ret.pre = lhs.pre;
        if (lhs.valid && lhs.r >= rhs.l)
            ret.pre = max(ret.pre, lhs.sum + rhs.pre);

        ret.suf = rhs.suf;
        if (rhs.valid && lhs.r >= rhs.l)
            ret.suf = max(ret.suf, lhs.suf + rhs.sum);
        ...
        if (lhs.r >= rhs.l)
            ret.ans = max(ret.ans, lhs.suf + rhs.pre);
        return ret;
    }
} t[N << 1];
