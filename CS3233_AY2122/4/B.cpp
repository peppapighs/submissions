        ...
        for( ; n; n >>= 1) {
            if(n & 1) {
                ...
                for(int k = 0; k < v; k++) for(int i = 0; i < v; i++) for(int j = 0; j < v; j++) {
                    if(ans[i][k] == -INF || A[k][j] == -INF) continue;
                    tmp[i][j] = max(tmp[i][j], ans[i][k] + A[k][j]);
                }
                for(int i = 0; i < v; i++) for(int j = 0; j < v; j++) ans[i][j] = tmp[i][j];
            }
            fill_n(tmp[0], N * N, -INF);
            for(int k = 0; k < v; k++) for(int i = 0; i < v; i++) for(int j = 0; j < v; j++) {
            ...
            for(int i = 0; i < v; i++) for(int j = 0; j < v; j++) A[i][j] = tmp[i][j]; 
        }
        long ret = -INF;
        for(int i = 0; i < v; i++) {
            for(int j = 0; j < v; j++) {
