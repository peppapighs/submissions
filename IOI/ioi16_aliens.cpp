...
struct cht {
    ...
    void add(long m, long b, int cnt) {
        line l(m, b, cnt);
        while(f.size() >= 2 && bad(f[f.size()-2], f.back(), l)) f.pop_back();
        f.emplace_back(l);
    }
    pii query(long x) {
        while(ptr + 1 < f.size() && f[ptr+1].get(x) < f[ptr].get(x)) ++ptr;
...
pii solve(long C) {
    ...
    for(int i = 1; i <= n; i++) {
        ...
        if(i == n) continue;
        hull.add(-2ll * (l[i+1] - 1), sq(l[i+1] - 1) + dp[i].x - sq(max(0ll, r[i] - l[i+1] + 1)), dp[i].y);
    }
    return dp[n];
}
...
long take_photos(int _n, int _m, int k, vector<int> _r, vector<int> _c) {
    ...
    for(pii p : coord) {
        if(p.x == r.back()) continue;
        while(!l.empty() && p.x >= r.back() && p.y <= l.back()) l.pop_back(), r.pop_back();
