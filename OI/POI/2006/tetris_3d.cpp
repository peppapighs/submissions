...
struct SegmentTree {
    ...
    int query(int x, int y, int p = 1, int l = 0, int r = S) {
        ...
        else if(mid + 1 <= x) return query(x, y, p << 1 | 1, mid + 1, r);
...
struct SegmentTree2d {
    ...
    void update(int x1, int y1, int x2, int y2, int k, int p = 1, int l = 0, int r = D) {
        t[p].update(x2, y2, k);
        if(x1 == l && r == y1) {
            lz[p].update(x2, y2, k);
            return;
        ...
        int mid = (l + r) >> 1;
        if(y1 <= mid) update(x1, y1, x2, y2, k, p << 1, l, mid);
        else if(mid + 1 <= x1) update(x1, y1, x2, y2, k, p << 1 | 1, mid + 1, r);
        else {
    ...
    int query(int x1, int y1, int x2, int y2, int p = 1, int l = 0, int r = D) {
        ...
        if(x1 == l && r == y1) return max(ret, t[p].query(x2, y2));
        int mid = (l + r) >> 1;
        if(y1 <= mid) ret = max(ret, query(x1, y1, x2, y2, p << 1, l, mid));
        else if(mid + 1 <= x1) ret = max(ret, query(x1, y1, x2, y2, p << 1 | 1, mid + 1, r));
        else {
