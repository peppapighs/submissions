...
int main() {
    ...
    for (int i = 1; i <= n; i++)
        scanf("%lld", h + i);
    for (int i = 1; i <= n; i++)
        scanf("%lld", w + i), pref[i] = (w[i] + pref[i - 1]) % M;
    ...
    for (int i = 1; i <= n; i++) {
        ...
        while (!stk.empty() && stk.back().y > h[i]) {
            pii now = stk.back();
            stk.pop_back();
            sum -= (pref[now.x] - pref[stk.back().x] + M) % M * (now.y + 1) %
                   M * now.y % M;
            sum = (sum + M) % M;
        }

        sum +=
            (pref[i] - pref[stk.back().x] + M) % M * (h[i] + 1) % M * h[i] % M;
        sum %= M;
        stk.emplace_back(i, h[i]);

        cur_ans = (cur_ans + (sum - (w[i] * (h[i] + 1) % M * h[i] % M) + M) %
                                 M * w[i] % M * i2 % M) %
                  M;
