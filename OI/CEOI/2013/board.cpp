...
void solve(char *S, vector<int> &bit) {
    ...
    for(int i = 1; S[i] != '\0'; i++) {
        if(S[i] == '1') bit.emplace_back(0), carry.emplace_back(0);
        if(S[i] == '2') bit.emplace_back(0), carry.emplace_back(1);
        if(S[i] == 'U') {
            int now = carry.back() / 2;
            bit.back() += carry.back() % 2;
            if(bit.back() < 0) bit.back() = 1, --now;
