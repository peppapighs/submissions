...
int add(pii k, int p, int l = 1, int r = coord.size()) {
    ...
    if (p == -1 || p > ptr)
        return newleaf(k);
    if (f(L, k) <= f(L, t[p].val) && f(R, k) <= f(R, t[p].val))
        return newpar(k, t[p].l, t[p].r);
    if (f(L, k) >= f(L, t[p].val) && f(R, k) >= f(R, t[p].val))
        return p;
    ...
    if (mid)
        swap(ret, k);
    if (lef != mid)
        return newpar(ret, add(k, t[p].l, l, m), t[p].r);
    else
        return newpar(ret, t[p].l, add(k, t[p].r, m + 1, r));
}
...
long get(int x, int p, int l = 1, int r = coord.size()) {
    ...
    if (p == -1 || p > ptr)
        return 1e18;
    if (x <= M)
        return min(f(x, t[p].val), get(x, t[p].l, l, m));
    else
        return min(f(x, t[p].val), get(x, t[p].r, m + 1, r));
}
...
void dfs(int u, int p, int d) {
    if (u != 1)
        ans[u] = get(V[u], ver[p]) + 1ll * d * V[u] + S[u];
    if (u != 1)
        ver[u] = add(pii(-d, u), ver[p]);
    else
        ver[u] = newleaf(pii(0, 0));
