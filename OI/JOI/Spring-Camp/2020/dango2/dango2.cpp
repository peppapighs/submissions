...
void update(int x, int k) { for(int i = x; i <= dango; i += i & -i) t[i] += k; }
...
void simulated_annealing() {
    for(int i = 1; i <= dango; i++) update(i, 1);
    double T = initT;

    int timer = 0;
    while(T >= 1e-6) {
        if(ans_cnt >= limit || cnt == dango) break;
        for(int k = 1; k <= 5; k++) {
            if(ans_cnt >= limit || cnt == dango) break;
            int u = get(range(1, dango - cnt));

            int add = 1;
            for(int v : g[u]) if(chk[v]) --add;
            int ncnt = cnt + add;
            ...
            if(valid) {
                chk[u] = 1, update(u, -1);
                for(int v : g[u]) if(chk[v]) chk[v] = 0, update(v, 1);
                cnt = ncnt;
                if(cnt > ans_cnt) {
                    ans_cnt = cnt;
                    ans = chk;
...
int main() {
    ...
    auto valid = [&](char a, char b) { return (a == 'P' && b == 'G') || (a == 'G' && b == 'P'); };
    ...
    for(int i = 1; i <= R; i++) for(int j = 1; j <= C; j++)
        for(int x = 0; x < V[i][j].size(); x++)
            for(int y = 0; y < x; y++)
                g[V[i][j][x]].emplace_back(V[i][j][y]), g[V[i][j][y]].emplace_back(V[i][j][x]);
    ...
    for(int i = 1; i <= dango; i++) if(ans[i]) {
        int r, c, t; tie(r, c, t) = pos[i];
        if(t == 0) B[r][c] = '|';
        if(t == 1) B[r][c] = '-';
        if(t == 2) B[r][c] = '\\';
        if(t == 3) B[r][c] = '/';
