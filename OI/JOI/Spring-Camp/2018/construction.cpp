...
long query(int u) {
    ...
    while (u) {
        ...
        for (pii x : Q[rot[u]]) {
            if (x.y < sz)
                now.emplace_back(x), sz -= x.y;
            else {
                now.emplace_back(x.x, sz);
                break;
            }
        }
        reverse(now.begin(), now.end());
        for (pii x : now)
            ret.emplace_back(x), coord.emplace_back(x.x);
        u = par[rot[u]];
    }
    ...
    auto upd = [&](int x, long k) {
        for (int i = x; i <= coord.size(); i += i & -i)
            t[i] += k;
    };
    auto que = [&](int x, long k = 0) {
        for (int i = x; i; i -= i & -i)
            k += t[i];
        return k;
    };
