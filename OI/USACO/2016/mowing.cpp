...
int get(vector<int> &coord, int x) { return lower_bound(coord.begin(), coord.end(), x) - coord.begin() + 1; }
...
struct Fenwick2D {
    ...
    int query2d(int x, int y1, int y2, int ret = 0) {
        for(int i = x; i; i -= i & -i) {
            int idx = upper_bound(coord[i].begin(), coord[i].end(), y1) - coord[i].begin();
            ret += query(t[i], idx) + query(t[i], t[i].size()-1) - query(t[i], get(coord[i], y2) - 1);
        }
        return ret;
    }
} tree;
