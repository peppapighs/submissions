...
struct line {
    ...
    friend bool operator<(const line &a, const line &b) { return a.c < b.c; }
};
...
template <typename T> void solve(vector<line> &vec, const T &f) {
    vector<line> hull;
    for (int i = 0; i < vec.size(); i++) {
        while (!hull.empty() && f(hull.back().m, vec[i].m))
            hull.pop_back();
        while (hull.size() > 1 &&
               bad(hull[hull.size() - 2], hull.back(), vec[i]))
            hull.pop_back();
        ...
        if (f(vec[i].c, A[Q[now]].c)) {
            ...
            while (l < r) {
                int mid = (l + r) >> 1;
                if (bad(hull[mid], hull[mid + 1], A[Q[now]]))
                    r = mid;
                else
                    l = mid + 1;
            }
            long hi = abs(A[Q[now]].c - hull[r].c),
                 lo = abs(A[Q[now]].m - hull[r].m);
            long g = __gcd(hi, lo);
