...
int main() {
    ...
    for(int i = 1; i <= coord.size(); i++) {
        ...
        for(pii p : Q[i]) {
            ans[p.y] = min(ans[p.y], coord[i-1] + p.x + t1.query(1, get(p.x)));
            ans[p.y] = min(ans[p.y], coord[i-1] - p.x + t2.query(get(p.x), coord.size()));
        }
    }
