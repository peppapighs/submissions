...
int main() {
    ...
    for(int i = 0; i < S.size(); i++) suf[i] = i, pos[i] = S[i];
    for(int k = 1; k <= S.size(); k <<= 1) {
        auto chk = [&](int a, int b) {
            if(pos[a] != pos[b]) return pos[a] < pos[b];
            if(a + k < S.size() && b + k < S.size()) return pos[a + k] < pos[b + k];
            return a > b;
        };
        sort(suf, suf + S.size(), chk);
        int tmp[N]; tmp[0] = 0;
        for(int i = 1; i < S.size(); i++) tmp[i] = tmp[i-1] + chk(suf[i-1], suf[i]);
        for(int i = 0; i < S.size(); i++) pos[suf[i]] = tmp[i];
    }
    for(int i = 0, k = 0; i < S.size(); i++) if(pos[i] != S.size() - 1) {
        for(int j = suf[pos[i] + 1]; S[i + k] == S[j + k] && S[i + k] != '#'; k++);
        lcp[N + pos[i]] = k;
        if(k) --k;
    }
    ...
    for(int i = 0; i < S.size(); i++) {
        if(id[suf[i]] == -1) continue;
        int mx = 0;
        if(L[i] != -1) mx = max(mx, get_lcp(L[i], i));
        if(R[i] != -1) mx = max(mx, get_lcp(R[i], i));
        if(i != S.size() - 1 && id[suf[i]] == id[suf[i + 1]]) mx = max(mx, get_lcp(i, i + 1));
