    ...
    for (pii a : ask) {
        ...
        if (a.x == 0) {
            ...
            for (int u = par[st], pv = st; u; pv = u, u = par[u]) {
                ...
                if (mx1[u].y == pv)
                    mx1[u] = max(mx1[u], now), now = pii(-1, -1);
                if (mx2[u].y == pv)
                    mx2[u] = max(mx2[u], now), now = pii(-1, -1);
                if (mx2[u] > mx1[u])
                    swap(mx1[u], mx2[u]);
                if (now > mx1[u])
                    swap(now, mx1[u]);
            ...
            for (int u = par[st], pv = st; u; pv = u, u = par[u])
                if (enable[u]) {
                    ...
                    if (mx1[u].y != pv && mx1[u].y != -1)
                        ans = max(ans, d[st][dep[u]] + mx1[u].x);
                    if (mx2[u].y != pv && mx2[u].y != -1)
                        ans = max(ans, d[st][dep[u]] + mx2[u].x);
                }
            printf("%d\n", ans);
        }
    }
