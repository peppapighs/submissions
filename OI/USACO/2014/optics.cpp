...
vector<pii> get_path(int x, int y, int dir) {
    ...
    while(true) {
        if(dir % 2 == 0) {
            int idx = lower_bound(mx[x].begin(), mx[x].end(), pair<int, char>(y, '#')) - mx[x].begin();
            int nex = idx + (dir ? -1 : 1);
            ...
            int idx = lower_bound(my[y].begin(), my[y].end(), pair<int, char>(x, '#')) - my[y].begin();
...
int sweep(vector<pair<int, pii> > vert, vector<pair<int, pii> > hori) {
    ...
    for(auto p : vert) coord.emplace_back(p.y.x), coord.emplace_back(p.y.y);
    for(auto p : hori) coord.emplace_back(p.x);
    ...
    for(auto e : event) {
        while(ptr < vert.size() && vert[ptr].x < e.x) {
            ret += tree.query(get(vert[ptr].y.y) - 1) - tree.query(get(vert[ptr].y.x));
            ++ptr; 
        }
...
int main() {
    ...
    for(auto &p : mx) sort(p.y.begin(), p.y.end());
    for(auto &p : my) sort(p.y.begin(), p.y.end());
    ...
    for(int i = 0; i < 4; i++) {
        ...
        int inter = sweep(vertical(lpath), horizontal(bpath)) + sweep(vertical(bpath), horizontal(lpath));
        if(bpath[0] == bpath.back()) ret += inter;
        else ret += 2 * inter;
    }
    printf("%d\n", ret / 2);
