...
static long hsh(vector<int> &v) {
    ...
    for(int i = 0; i < v.size(); i++) ret = ret * iden + v[i];
    return ret;
}
...
static void gen_permute(int u) {
    if(v.size() == k) {
        for(int i = 1; i <= n; i++) if(find(v.begin(), v.end(), i) == v.end()) {
            vector<int> now = v;
            now.emplace_back(i);
...
vector<int> findOriginalCards(vector<int> cards) {
    int now = M1[hsh(cards)];
    vector<int> ret;
    for(int i : cards) if(i != now) ret.emplace_back(i);
    return ret;
}