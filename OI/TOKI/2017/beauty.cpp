...
void tarjan(int u, int p) {
    ...
    for(int v : g[u]) if(v != p) {
        if(!pre[v]) {
            ...
            if((!p && child > 1) || (p && low[v] >= pre[u])) art[u] = true;
            if(low[v] >= pre[u]) {
...
void gen_bct() {
    ...
    for(vector<int> &c : cc) {
        g.emplace_back(vector<int>());
        for(int v : c) {
            if(art[v]) g[g.size()-1].emplace_back(id[v]), g[id[v]].emplace_back(g.size()-1);
            else id[v] = g.size() - 1;
...
void gen_lca(int u, int p) {
    ...
    for(int v : g[u]) if(v != p) gen_lca(v, u);
}
