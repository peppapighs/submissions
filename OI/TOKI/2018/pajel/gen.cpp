...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        char S[100];
        scanf(" %s", S + 1);
        if(S[1] == '0') for(int j = 1; j <= n; j++) a[j][i] = -1;
        else if(S[1] != '-') {
            int now = strlen(S + 1), val = 0;
            for(int j = 1; j < now; j++) val = val * 10 + (S[j] - '0');
            for(int j = 1; j < val; j++) a[j][i] = -1;
    ...
    for(int i = 1; i <= n; i++) {
        char S[100];
        scanf(" %s", S + 1);
        if(S[1] == '0') for(int j = 1; j <= n; j++) a[i][j] = -1;
        else if(S[1] != '-') {
            int now = strlen(S + 1), val = 0;
            for(int j = 1; j < now; j++) val = val * 10 + (S[j] - '0');
            for(int j = 1; j < val; j++) a[i][j] = -1;
        ...
        if(S[1] == '0') for(int j = 1; j <= n; j++) a[i][j] = -1;
            ...
            int now = strlen(S + 1), val = 0;
            for(int j = 1; j < now; j++) val = val * 10 + (S[j] - '0');
            for(int j = 1; j < val; j++) a[i][n - j + 1] = -1;
    ...
    for(int i = 1; i <= n; i++) {
        ...
        if(S[1] == '0') for(int j = 1; j <= n; j++) a[j][i] = -1;
            ...
            int now = strlen(S + 1), val = 0;
            for(int j = 1; j < now; j++) val = val * 10 + (S[j] - '0');
            for(int j = 1; j < val; j++) a[n - j + 1][i] = -1;
            add(n - val + 1, i, S[now] == 'M' ? 1 : 2);
        }
    }
    for(int i = 1; i <= n * n; i++) spar[i] = par[i];
    for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++) g[i][j] = a[i][j];
    scc = cc;
    while(1) {
        if(cc <= p) answer(a);
        vector<pii> M, B;
        for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++) if(par[f(i, j)] == f(i, j)) {
            if(a[i][j] == 1) M.emplace_back(i, j);
            else if(a[i][j] == 2) B.emplace_back(i, j);
        }
        int mmx = cc;
        pii ma, mb;
        shuffle(M.begin(), M.end(), dre);
        shuffle(B.begin(), B.end(), dre);
        if(uid(dre) % 2) swap(M, B);
        
        int tmp[N][N], tpar[N * N], tcc = cc;
        for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++) tmp[i][j] = a[i][j];
        for(int i = 1; i <= n * n; i++) tpar[i] = par[i];
        for(int i = 0; i < M.size(); i++) for(int j = i + 1; j < M.size(); j++) {
            ...
            while(!Q.empty()) {
                ...
                for(int i = 0; i < 4; i++) {
                    ...
                    if(nr < 1 || nr > n || nc < 1 || nc > n) continue;
                    if((a[nr][nc] == 0 || a[nr][nc] == a[st.x][st.y]) && d[now.x][now.y] + 1 < d[nr][nc]) {
            ...
            // print(a);
            for(int i = 1; i <= n * n; i++) ::par[i] = tpar[i];
            for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++) a[i][j] = tmp[i][j];
            cc = tcc;
        }
        if(mmx == cc) {
            for(int i = 1; i <= n * n; i++) ::par[i] = spar[i];
