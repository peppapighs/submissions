...
template<typename T>
void bfs(int sr, int sc, int d[N][N], const T &f) {
    fill_n(d[0], N * N, 1e9);
    ...
    while(!Q.empty()) {
        ...
        int r, c, dist; tie(r, c, dist) = now;
        if(d[r][c] != dist) continue;
        for(int i = 0; i < 4; i++) {
            int nr = r + dx[i], nc = c + dy[i];
            if(border(nr, nc) || A[nr][nc] == 'I') continue;
...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        scanf(" %s", A[i] + 1);
        for(int j = 1; j <= m; j++) {
            if(A[i][j] == 'Y') sr = i, sc = j, A[i][j] = '.';
            if(A[i][j] == 'T') er = i, ec = j, A[i][j] = '.';
            if(A[i][j] == 'V') vr = i, vc = j, A[i][j] = '.';
        }
    }
    ...
    bfs(sr, sc, ds, [&](int x, int y, int d) {
        for(int i = 0; i < 4; i++) {
            ...
            while(!border(r, c)) {
                if(A[r][c] == 'I') break;
                if(dv[r][c] <= d) return true;
                r += dx[i], c += dy[i];
            }
        }
        return false;
    });
    if(ds[er][ec] != 1e9) printf("YES\n");
    else printf("NO\n");
