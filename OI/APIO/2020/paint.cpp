...
int minimumInstructions(int _n, int _m, int _k, vector<int> C, vector<int> A, vector<vector<int> > B) {
    ...
    n = _n, m = _m, k = _k;
    C.insert(C.begin(), 0), A.insert(A.begin(), 0), B.insert(B.begin(), vector<int>());

    for(int i = 1; i <= m; i++) for(int x : B[i])
    ...
    for(int i = 1; i <= n; i++) {
        ...
        for(int idx = 0; idx < col[C[i]].size(); idx++) {
            int x = col[C[i]][idx];
            if(x != 1) {
                while(j + 1 <= col[C[i - 1]].size() && col[C[i - 1]][j] <= x - 1)
                    ++j;
                if(j > col[C[i - 1]].size() || col[C[i - 1]][j - 1] != x - 1)
                    continue;
                cur[idx + 1] = pre[j] + 1;
            } else {
                if(col[C[i - 1]].empty() || col[C[i - 1]].back() != m) continue;
