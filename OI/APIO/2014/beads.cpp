...
2 - Parent edge any color, 1 node in subtree with 2 blue edges directed to its
children 3 - Parent edge must be blue, 1 node in subtree with 2 blue edges
directed to its children
*/
...
void dfs(int u, int p) {
    ...
    for (int i = 0; i < min(2, (int)D1.size()); i++)
        for (int j = 0; j < min(2, (int)D2.size()); j++)
            if (D1[i].y != D2[j].y)
                dp[u][2] = max(dp[u][2], dp[u][0] + D1[i].x + D2[j].x);

    // Directed 2 blue edges to 2 0-child
    if (D1.size() > 1)
        dp[u][2] = max(dp[u][2], dp[u][0] + D1[0].x + D1[1].x);
    ...
    for (int i = 0; i < M.size(); i++)
        dp[u][2] = max(dp[u][2],
                       dp[u][0] - M[i] + max(dp[V[i]][2], dp[V[i]][3] + W[i]));
}
