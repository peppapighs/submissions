...
function<pii(int, int)> f[4] = {[](int x, int y) { return pii(x, y); },
...
void solve(int idx, int n, int m) {
    ...
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= m; j++) {
            ...
            tie(x, y) = f[idx](i, j);
            pref[i][j] =
                pref[i - 1][j] + pref[i][j - 1] - pref[i - 1][j - 1] + A[x][y];
        }
    for (int i = k; i <= n; i++)
        for (int j = k; j <= m; j++)
            for (int t = 1; t <= 3; t++) {
                dp[i][j][t] = max(dp[i - k][m][t - 1], dp[i][j - k][t - 1]) +
                              rect(i - k + 1, i, j - k + 1, j);
                dp[i][j][t] =
                    max({dp[i][j][t], dp[i - 1][j][t], dp[i][j - 1][t]});
            }
