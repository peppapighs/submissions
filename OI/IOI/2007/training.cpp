...
void dfs(int u, int p) {
    int mx = 1 << g[u].size();
    for(int i = 0; i < g[u].size(); i++) if(g[u][i] != p) pos[g[u][i]] = i;
    for(int v : g[u]) if(v != p) {
        dfs(v, u);
        for(int i = 0; i < mx; i++) if(i >> pos[v] & 1)
            dp[u][i] = max(dp[u][i], dp[u][i ^ (1 << pos[v])] + dp[v][(1 << g[v].size()) - 1]);
    }
    for(iii e : res[u]) {
        ...
        while(a != u) {
            ...
            if(pv_a != -1) sum_a += dp[a][mx_a ^ (1 << pos[pv_a])];
            else sum_a += dp[a][mx_a];
            pv_a = a, a = par[a][0];
        }
        while(b != u) {
            int mx_b = (1 << g[b].size()) - 1;
            if(pv_b != -1) sum_b += dp[b][mx_b ^ (1 << pos[pv_b])];
            else sum_b += dp[b][mx_b];
            pv_b = b, b = par[b][0];
        }
        int x = 0;
        if(pv_a != -1) x ^= 1 << pos[pv_a];
        if(pv_b != -1) x ^= 1 << pos[pv_b];
        for(int i = 0; i < mx; i++) if((x & i) == x)
