...
int main() {
    ...
    for(int i = 1; i <= H; i++) for(int j = 1; j <= W; j++) for(int k = i; k; k--) {
        int l = 1, r = j;
        while(l < r) {
            int mid = (l + r + 1) >> 1;
            if(rect(k, i, mid, j) >= K) l = mid;
            else r = mid - 1;
        }
        if(rect(k, i, l, j) == K) {
    ...
    for(int i = 1; i <= H; i++) mnr[i] = min(mnr[i], mnr[i - 1]);
    for(int i = 1; i <= W; i++) mnc[i] = min(mnc[i], mnc[i - 1]);

    int ans = 1e9;
    for(int i = H; i; i--) for(int j = W; j; j--) for(int k = i; k <= H; k++) {
        int l = j, r = W;
        while(l < r) {
            int mid = (l + r) >> 1;
            if(rect(i, k, j, mid) >= K) r = mid;
            else l = mid + 1;
        }
        if(rect(i, k, j, r) == K) {
