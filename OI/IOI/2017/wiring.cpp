...
long min_total_length(vector<int> r, vector<int> b) {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        if (all[i].y == 0) {
            ...
            if (it != b.end())
                dp[i] = min(dp[i], dp[i - 1] + *it - all[i].x);
            if (it != b.begin())
                dp[i] = min(dp[i], dp[i - 1] + all[i].x - *--it);
        } else {
        ...
        if (pre[i] != 1) {
            ...
            dp[i] =
                min(dp[i],
                    query(t1, max(l, pre[i] - sz_l[i]), pre[i] - 1) + sum_l[i] +
                        1ll * sz_l[i] * (all[pre[i]].x - all[pre[i] - 1].x));
            dp[i] = min(dp[i], query(t2, l, pre[i] - sz_l[i] - 1) + sum_l[i]);
        }
        update(t1, i, dp[i - 1] + sum_r[i]);
        update(t2, i,
               dp[i - 1] + sum_r[i] +
                   1ll * sz_r[i] * (all[pos[i] + 1].x - all[pos[i]].x));
    }
