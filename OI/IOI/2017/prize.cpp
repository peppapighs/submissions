...
void solve(int l, int r, vector<int> &L, vector<int> &R, int &ans) {
	if(r - l <= 1 || make_pair(L[0], L[1]) == make_pair(R[0], R[1]) || ~ans) return;

	int mid = (l + r) >> 1;
	vector<int> M = query(mid);
	if(valid(M)) return void(ans = mid);

	if(L[0] + L[1] == R[0] + R[1] && M[0] + M[1] < L[0] + L[1] && L[0] == R[0] - 1)
		return;
