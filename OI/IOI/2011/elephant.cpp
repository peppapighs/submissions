...
void solve_bucket(int idx) {
    ...
    for (int i = vec.size() - 1, j = vec.size(); ~i; i--) {
        while (vec[j - 1] > vec[i] + k)
            --j;
        if (j < vec.size())
            jump[idx][i] = jump[idx][j], cnt[idx][i] = cnt[idx][j] + 1;
        else
            jump[idx][i] = vec[i] + k, cnt[idx][i] = 1;
    }
}
...
void add_elephant(int x, bool b) {
    ...
    while (R[++ptr] < x)
        ;
    int idx = lower_bound(bucket[ptr].begin(), bucket[ptr].end(), x) -
              bucket[ptr].begin();
...
int update(int i, int y) {
    ...
    if (!--mp[pos[i]])
        add_elephant(pos[i], 0), mp.erase(pos[i]);
    if (!mp[y])
        add_elephant(y, 1);
    ++mp[y], pos[i] = y;
    ...
    int answer = 0, cur = -1;
    for (int i = 0; i < bcnt; i++) {
        int idx = upper_bound(bucket[i].begin(), bucket[i].end(), cur) -
                  bucket[i].begin();
