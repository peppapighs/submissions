...
int update(int x, int p, int l = 0, int r = N - 1) {
    if (l == r)
        return newleaf(t[p] + 1);
    int mid = (l + r) >> 1;
    if (x <= mid)
        return newpar(update(x, L[p], l, mid), R[p]);
    else
        return newpar(L[p], update(x, R[p], mid + 1, r));
}
...
int remove(int k, int p, int pz, int l = 0, int r = N - 1) {
    ...
    if (l > k)
        return p;
    int mid = (l + r) >> 1;
    return newpar(remove(k, L[p], L[pz], l, mid),
                  remove(k, R[p], R[pz], mid + 1, r));
}

int choose(int k, int all, int p, int l = 0, int r = N - 1) {
    if (l == r)
        return newleaf(t[p] + k);
    int sum = t[L[all]] - t[L[p]], mid = (l + r) >> 1;
    if (sum >= k)
        return newpar(choose(k, L[all], L[p], l, mid), R[p]);
    else
        return newpar(L[all], choose(k - sum, R[all], R[p], mid + 1, r));
}
...
void init(int _n, int A[], int B[]) {
    n = _n, rot[0] = build();
    for (int i = 0; i < n; i++)
        coord[A[i]].emplace_back(B[i]);
    for (int i = 1; i <= n; i++) {
        rot[i] = remove(i - 1, rot[i - 1], rot[0]);
        for (int b : coord[i])
            rot[i] = update(b, rot[i]);
    }
    start = build();
}
