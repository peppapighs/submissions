...
int update(int x, int k, int p, int l = 1, int r = re) {
    if (l == r)
        return newleaf(vec[p].d + k);
    int mid = (l + r) >> 1;
    if (x <= mid)
        return newpar(update(x, k, vec[p].l, l, mid), vec[p].r);
    else
        return newpar(vec[p].l, update(x, k, vec[p].r, mid + 1, r));
}
...
int get_val(int x, int p, int l = 1, int r = re) {
    ...
    int mid = (l + r) >> 1;
    if (x <= mid)
        return get_val(x, vec[p].l, l, mid);
    else
        return get_val(x, vec[p].r, mid + 1, r);
}
    ...
    for (int i = 1, a, b; i <= q; i++) {
        ...
        if (id[a] && id[b])
            ...
            if (!id[a])
                for (int u : emp[a])
                    ans +=
                        upper_bound(euler[b].begin(), euler[b].end(), out[u]) -
