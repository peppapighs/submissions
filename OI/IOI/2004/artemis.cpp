...
int main() {
	...
	for(int i = 1; i <= n; i++) {
		x[i] = lower_bound(coordx.begin(), coordx.end(), x[i]) - coordx.begin() + 1;
		y[i] = lower_bound(coordy.begin(), coordy.end(), y[i]) - coordy.begin() + 1;
	}
	...
	for(int i = 1; i <= n; i++) {
		...
		for(int j = i + 1; j <= n; j++) {
			...
			if(val < ans && val >= m) ans = val, a1 = pos[i], a2 = pos[j];
			if(ans == m) break;
		}
		if(ans == m) break;
	}
