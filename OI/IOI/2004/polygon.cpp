...
pii get_min(vector<pii> &vec) {
    int mnx = 1e9, mny = 1e9;
    for(pii p : vec) mnx = min(mnx, p.x), mny = min(mny, p.y);
    return pii(mnx, mny);
}
...
vector<pii> subtract(vector<pii> &P, vector<pii> &A) {
    ...
    for(pii p1 : P) {
        ...
        for(pii p2 : A) {
            ...
            if(vec1 == vec2) {
                valid = true;
                if(p1 != p2) B.emplace_back(p1.x - p2.x, p1.y - p2.y);
                break;
            }
        }
        if(!valid) B.emplace_back(p1);
    }
    return B;
}
...
vector<pii> get_coord(vector<pii> &vec) {
    ...
    for(pii p : vec) ret.emplace_back(ret.back().x + p.x, ret.back().y + p.y);
    ret.pop_back();

    pii mn = get_min(ret);
    for(pii &p : ret) p.x -= mn.x, p.y -= mn.y;
    return ret;
}
...
int main() {
    ...
    vert.resize(n);
    for(int i = 0; i < n; i++) scanf("%d %d", &vert[i].x, &vert[i].y);
    for(int i = 0; i < n; i++) {
        int x = vert[(i + 1) % n].x - vert[i].x, y = vert[(i + 1) % n].y - vert[i].y;
        int g = abs(__gcd(x, y));
        
        for(int j = 1; j <= g; j++) E.emplace_back(x / g * j, y / g * j);
        P.emplace_back(x, y);
    }
    sort(E.begin(), E.end());
    ...
    function<bool()> solve_tri = [&]() {
        for(int i = 0; i < (int)E.size(); i++) {
            ...
            for(int j = i + 1; j < (int)E.size(); j++) {
                ...
                if(it == E.end() || *it != pii(-now.x, -now.y)) continue;
    ...
    function<bool()> solve_qua = [&]() {
        vector<pii> tmp;
        for(int i = 0; i < (int)E.size(); i++) for(int j = i + 1; j < (int)E.size(); j++)
        ...
        for(int i = 0; i < (int)E.size(); i++) {
            bool valid = false;
            for(int j = i + 1; j < (int)E.size(); j++) {
                if(get_prim(E[i]) == get_prim(E[j])) continue;
                pii now(E[i].x + E[j].x, E[i].y + E[j].y);
                if(now.x == 0 && now.y == 0) continue;

                auto it = lower_bound(tmp.begin(), tmp.end(), pii(-now.x, -now.y));
                if(it == tmp.end() || *it != pii(-now.x, -now.y)) continue;

                A.emplace_back(E[i]), A.emplace_back(E[j]);
                for(int k = 0; k < (int)E.size(); k++) if(k != i && k != j) {
                    bool found = false;
                    for(int l = k + 1; l < (int)E.size(); l++) if(l != i && l != j && E[k].x + E[l].x == -now.x && E[k].y + E[l].y == -now.y) {
                        if(get_prim(E[k]) == get_prim(E[l])) continue;
                        if(cross(A[0], A[1]) != cross(E[k], E[l]))
                            A.emplace_back(E[l]), A.emplace_back(E[k]);
                        else A.emplace_back(E[k]), A.emplace_back(E[l]);
                        found = true;
                        break;
                    }
                    if(found) break;
    ...
    if(solve_qua()) return 0;
    A.clear();
    if(solve_tri()) return 0;
    A.clear();
    solve_vec();
