...
public:
    ...
    void assign(int i, int j, long k) {
        A[i][j] = k;
    }

    long hungarian() {
        u.assign(n + 1, 0), v.assign(m + 1, 0), p.assign(m + 1, 0), way.assign(m + 1, 0);
        for(int i = 1, j0; i <= n; i++) {
            ...
            do {
                used[j0] = true;
                long delta = INF, j1;
                for(int j = 1, i0 = p[j0]; j <= m; j++) if(!used[j]) {
...
int main() {
    ...
    for(int i = 1; i <= n; i++) scanf("%d %d", &wine[i].x, &wine[i].y);
    for(int i = 1; i <= m; i++) scanf("%d %d", &cour[i].x, &cour[i].y);
