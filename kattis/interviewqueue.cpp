...
int main() {
    ...
    while(!candid.empty()) {
        ...
        vector<int> change;
        for(int x : L) change.emplace_back(unite(x, l[x]));
        for(int x : R) change.emplace_back(unite(x, r[x]));

        set<int> S;
        for(int x : change) if(find(x) == x)
    ...
    for(int i = 1; i <= n; i++) if(find(i) == i) printf("%d ", A[i]);
    printf("\n");
