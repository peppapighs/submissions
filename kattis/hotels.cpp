...
void solve() {
    ...
    int ans = 0, u = 0;
    for(int i = 0; i < coord.size() - 1; i++) for(int d : {-1, 0, 1}) {
        int pos = ((-dist[i] + dist[i + 1] + coord[i] + coord[i + 1]) / 2) + d;
        if(pos < coord[i] || coord[i + 1] < pos) continue;
        int ret = min(pos - coord[i] + dist[i], coord[i + 1] - pos + dist[i + 1]);
        if(ans < ret)
            ans = ret, u = pos;
    }
    printf("%d %d\n", ans, u);
