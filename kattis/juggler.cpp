...
void update(pitem t) { if(t) t->sz = sz(t->l) + 1 + sz(t->r); }
...
int order(pitem t, int key, int add = 0) {
    if(!t) return 0;
    if(t->val == key) return add + sz(t->l) + 1;
    else if(t->val < key) return order(t->r, key, add + sz(t->l) + 1);
    else return order(t->l, key, add);
}
