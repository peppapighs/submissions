...
void gen_dir(pii now, int idx) {
    if(idx < 4) {
        ...
        for(int i = 0; i < dx[idx]; i++) {
            ++it;
            if(it == vecX.end()) return;
        }
        for(int i = 0; i > dx[idx]; i--) {
            if(it == vecX.begin()) return;
            --it;
        }
        ...
        for(int i = 0; i < dy[idx]; i++) {
            ++it;
            if(it == vecY.end()) return;
        }
        for(int i = 0; i > dy[idx]; i--) {
            if(it == vecY.begin()) return;
            --it;
        }
...
int main() {
    ...
    for(auto &p : X) sort(p.y.begin(), p.y.end());
    for(auto &p : Y) sort(p.y.begin(), p.y.end());
    ...
    while(!Q.empty()) {
        ...
        if(coord[now - 1] == pii(c, d))
            return !printf("%d\n", dist[now]);
        for(int v : g[now]) if(dist[v] > dist[now] + 1) {
            dist[v] = dist[now] + 1;
            Q.emplace(v);
        }
    }
